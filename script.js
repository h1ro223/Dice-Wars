...
$('btn-settings').addEventListener('click',()=>{AudioSys.playSe('click');$('settings-modal').classList.remove('hidden')});
...
function onP1Click(i){
    ...
    else{const n=GS.rolledDice.filter(x=>x.selected).length;if(n>=GS.rolledDice.length)return;d.selected=true;AudioSys.playSe('select');}
...
function onP2Click(i){
    ...
    if(d.selected){d.selected=false;AudioSys.playSe('deselect');}
    else{const n=GS.rolledDiceP2.filter(x=>x.selected).length;if(n>=GS.rolledDiceP2.length)return;d.selected=true;AudioSys.playSe('select');}
